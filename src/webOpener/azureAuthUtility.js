class AzureAuthManager {
  ...
  async getAccessToken(scopes) {
    console.log(`Getting access token of scopes [${scopes}]...`);
    const session = await this.getAzureAuthSession(scopes);
    console.log("Token retrieved of scopes [${scopes}]");
  ...
  async getAzureAuthSession(scopes) {
    ...
    const newSessions = await this.microsoftAuthentication.getSessions(scopes, {
